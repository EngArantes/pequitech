...
const Header = () => {
  ...
  const { filterNewsByCategory } = useNews();
    ...
    <header className="header-container">
      <nav className="nav-links">
        <Link to="/" className="logo-container">
          <img className="logomarca-header" src={Logo} alt="Logomarca" />
        </Link>
        ...
        <div className={`menu-items ${isMenuOpen ? 'open' : ''}`}>
          ...
          {currentUser ? (
            ...
            <button className="icon-button" onClick={toggleModal} aria-label="Login">
              <FaUser size={20} />
            </button>
          )}
        </div>
